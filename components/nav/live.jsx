...
useEffect(()=>{
			axios.get("https://apiv3.apifootball.com/?action=get_events&from="+today_date+"&to="+today_date+"&timezone="+getTimeZone()+"&match_live =1&APIkey="+api_key)
			.then(res=>{
						...
						var important = []
						var data = res.data
						let m = data.filter((item)=> item.match_live === "1")
			 			console.log(res)
                        leagues_ids.forEach((element)=>{
                            var filter = m.filter((item)=> item.league_id== element)
                            console.log(filter)
                            filter.map((item)=>{
                                 important.push(item)
                            })
