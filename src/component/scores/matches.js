...
function Matches(){
    ...
    var [pinned2, setpinned2] = useState({name : "", crest : ""})
    ...
    async function pushe(item){
    await fetch("http://localhost:6100/pinned_recieve/"+login_email, {
        method : "post",
...
async function fetcher1(){
            ...
            try {
                ...
                const result1 =await fetch("http://localhost:6100/matches"); 
                ...
                more1= main1.map((item)=>{
                    ...
                    else if (item.match_status == "Cancelled"){
...
<div><p style = {{backgroundColor : "rgb(2, 2, 34)"}} onClick = {()=> pushe(item)}>🔕</p></div>

  <div style = {{width : "100%"}} id = "lets">  
     <Link to={"/personal"} state={item} style={{textDecoration : "none", color : "none", width : "90%"}}>
                                                              
                          <div className="indi_matches">


                                  <div className="time_and_love">
                                  <h6 style={{color : "warning"}}>           {status}'|
                              </h6>
  ...
  <div className="teams_names" style = {{width : "100%"}}>

        <div style  = {{display : "flex", justifyContent : "space-between"}}>
        <div style ={{display : "flex"}}><img src={item.team_home_badge} style={{width : "20px", height : "20px"}}></img> <h6>{item.match_hometeam_name}</h6> </div>
        <div><h6 style = {{color : "gold"}}>{item.match_hometeam_score}</h6></div>
        </div>        


        <div style  = {{display : "flex", justifyContent : "space-between"}} >
        <div style ={{display : "flex"}}><img src={item.team_away_badge}style={{width : "20px", height : "20px"}}></img><h6>{item.match_awayteam_name}</h6> </div>
        <div><h6 style = {{color : "gold"}}>{item.match_awayteam_score}</h6></div>
        </div> 
...
setsaudi(sui.map((item)=>{
  ...
  return(
      ...
      setRet(
        <>
        <div>
<div style = {{display : "flex"}}>{pinned2.crest} <h3 className = "text-light"> {pinned2.name} </h3>
</div>
{pin}
...
{others1}
                ...
                setRet(<h3 className="text-center text-danger">...Network Connection Problem...</h3>)
                console.log(error)
          }
...
useEffect(()=>{
    ...
    return(
        <body style={{maxHeight : window.innerHeight - 150}}>
