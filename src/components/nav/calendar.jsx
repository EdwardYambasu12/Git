...
export default function Calendar(props) {
  ...
  useEffect(() => {
    if (userFavorites && leagues.length > 0) {
      const pinnedMatchIds = new Set(userFavorites.pinned_matches.map(item => JSON.parse(item).id));
      const newPinnedStatus = {};

      leagues.forEach(item => {
        item.matches.forEach(match => {
          newPinnedStatus[match.id] = pinnedMatchIds.has(match.id);
        });
      });

      setMatchPinnedStatus(newPinnedStatus);
    }
  }, [userFavorites, leagues]);

  const togglePin = async (match) => {
    const isPinned = matchPinnedStatus[match.id] || false;
    const url = isPinned ? `${Line}/unpinned_matches` : `${Line}/pinned_matches`;
    ...
    return leagues.map((item, index) => (
      <Accordion key={index} defaultExpanded sx={{ borderRadius: '15px' }}>
        <AccordionSummary expandIcon={<ExpandMoreIcon />} aria-controls="panel1-content" id="panel1-header">
          <div className="league_description" onClick={() => { 
              navigate("leagues/" + item.primaryId); 
              sessionStorage.setItem("selected_league", JSON.stringify(item)); 
        ...
        <AccordionDetails>
          {item.matches.map((match, matchIndex) => {
            ...
            if (!match.status.started) {
              ...
              status = <div style={{ display: "flex", justifyContent: "space-between", width: "100%" }}><h6>{match.home.score}</h6><h6>-</h6><h6>{match.away.score}</h6></div>;
              live = <h6 style={{ width: "20px", fontSize: "0.7em", display: "flex", justifyContent: "center", alignItems: "center", color: "white", borderRadius: "50%", background: "red" }}>{match.status.liveTime.short}</h6>;
            } else {
              status = <div style={{ display: "flex", justifyContent: "space-between", width: "100%" }}><h6>{match.home.score}</h6><h6>-</h6><h6>{match.away.score}</h6></div>;
              live = <h6 style={{ width: "20px", textAlign: "center", alignItems: "center", color: "black", borderRadius: "50%", background: "#EEEEEE" }}>FT</h6>;
            }
              ...
              <div key={matchIndex} style={{ display: "flex", marginTop: "3%", width: "100%", justifyContent: "space-between", background: "white", borderRadius: "10px", textDecoration: "none" }}>
                <div style={{ display: "flex", justifyContent: "space-between", width: "100%", height: "50px", alignItems: "center" }}>
                  ...
                  <Link to={`result/${match.id}`} style={{ display: "flex", textDecoration: "none", justifyContent: "space-between", width: "90%" }}>
                    ...
                    <div className="text-dark" style={{ width: "20%", display: "flex", justifyContent: "center", textAlign: "center", color: "black" }}>
