...
const Table = ({ props }) => {
  ...
  const [groups, setGroups] = useState({}); // State to hold grouped teams
  ...
  useEffect(() => {
    ...
    try {
      ...
      const teamData = Array.from(teamElements).map((team) => ({
        ...
        points: parseInt(team.getAttribute('p'), 10),
        wins: parseInt(team.getAttribute('w'), 10),
        draws: parseInt(team.getAttribute('d'), 10),
        losses: parseInt(team.getAttribute('l'), 10),
        goalsFor: parseInt(team.getAttribute('g'), 10),
        goalsAgainst: parseInt(team.getAttribute('c'), 10),
        group: team.getAttribute('group') || 'Unassigned', // Assuming 'group' attribute
      }));
      ...
      setTeams(teamData);
       ...
       const grouping = xmlDoc.getElementsByTagName("subt")
      const group = Array.from(grouping).map((team)=>({
        group : team.getAttribute("name"), 
        job : team.getElementsByTagName("t"),
    ...
    group.map((item)=>{
        ...
        var dunk = []
        Array.from(item.job).map((id)=>{
            dunk.push(id.getAttribute("id"))

            const mid = id.getAttribute("id")

            const filterer = teamData.filter((it)=> it.id === mid)
            const dat = filterer[0]

            dat.group = item.group
...
const groupedTeams = monk.reduce((acc, team) => {
        if (!acc[team.group]) {
          acc[team.group] = [];
        }
      ...
      console.log(teamData, monk )
      console.log(groupedTeams, "group")
      ...
      const cRule = rules.querySelector('ti[tkey="championsleague"]');
      const eRule = rules.querySelector('ti[tkey="europaleague"]');
      const miht = rules.querySelector('subt[name = "Grp."]')
        console.log(miht, "miht")
      // Process rules if needed (not shown in this snippet)
          ...
          <table>
            ...
            <tbody>
              {groups[groupName].map((team, index) => (
                <tr
                  key={team.id}
                  style={{ width: "100%", height: "50px", fontSize: '0.7em', fontWeight: "bold", alignItems: "center" }}
                >
                  ...
                  <td style={{ width: "10%" }}>{team.goalsFor - team.goalsAgainst}</td>
                  <td style={{ width: "10%" }}>{team.points}</td>
                </tr>
              ))}
            </tbody>
