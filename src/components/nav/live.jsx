...
console.log(getTimeZone())
	...
	const api_key = api
	const [main_matches, setMain_matches] = useState(<> <Box sx={{ width: window.innerWidth }}>
      <Skeleton />
...
useEffect(()=>{
				axios.get("https://apiv3.apifootball.com/?action=get_events&match_live=1&from="+today_date+"&to="+today_date+"&withPlayerStats=1&timezone="+getTimeZone()+"&APIkey="+api_key)
			.then(res=>{
			        ...
			        const details = item.list
					        ...
					        >
					          ...
					          <div style = {{display: "flex", alignItems : "center", width : "90%"}}><img src = {state} id = "logod"></img><h6 id = "break-down1" >{item.league_name}</h6></div>
                            ...
                            <div onClick={()=>{sed(id)}}   to = {"result/"+id.match_id} style = {{display : "flex", marginTop : "3%", width : "100%", justifyContent : "space-between", background : "white", borderRadius : "10px", textDecoration : "none"}} >
                                    ...
                                    <div style = {{display : "flex", width : "33%",justifyContent : "space-between", display : "flex", alignItems : "center", }}><h6 className = "text-dark" style ={{textOverflow : "ellipsis", whiteSpace : "nowrap", overflow : "hidden"}}>{id.match_hometeam_name}</h6>{radar}<img src = {id.team_home_badge} style = {{width : "30px", height : "30px"}}></img></div>
                                    ...
                                    <div style = {{display : "flex", width : "33%", justifyContent : "space-between", display : "flex", alignItems : "center", }}><img src = {id.team_away_badge} style = {{width : "30px", height : "30px"}}></img>{radar1}<h6 className = "text-dark" style ={{textOverflow : "ellipsis", whiteSpace : "nowrap", overflow : "hidden", fontSize : "1em"}}>{id.match_awayteam_name}</h6></div>
                                    </Link>
                                    </div>
                                    <div></div>
	...
	return(
<div style = {{background : "#EEEEEE", height : window.innerHeight-120, overflowY : "auto", overflowX : "hidden" }}>	
